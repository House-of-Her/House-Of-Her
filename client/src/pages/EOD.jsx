  ...
  if (user?.role === 'model') {
    return (
      <div className="p-8 text-center text-gray-500">
        Models cannot view EOD reports.
      </div>
    );
  }
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {showForm && (
        <form onSubmit={submit} className="bg-white border border-pink-100 rounded-xl p-6 mb-8 shadow-sm space-y-4">
          ...
          <div>
            ...
            <textarea
              rows={2}
              value={form.important_followups}
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {reports.map(r => (
            <div key={r.id || Math.random()} className="bg-white border border-pink-100 rounded-xl p-5 shadow-sm">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h3 className="font-semibold text-pink-700">{r.chatter_name || 'Unknown'}</h3>
                  <p className="text-sm text-gray-500">
                    {r.report_date}
                    {r.shift_start && ` • ${r.shift_start} - ${r.shift_end || '?'}`}
                  </p>
                </div>
                <span className="text-lg font-bold text-green-600">
                  ${r.overall_earnings || 0}
                </span>
              </div>
              {r.models_worked && (
                <p className="text-sm mb-1">
                  <span className="font-medium">Models:</span> {r.models_worked}
                </p>
              )}
              {r.earnings_breakdown && (
                <p className="text-sm mb-1">
                  <span className="font-medium">Breakdown:</span> {r.earnings_breakdown}
                </p>
              )}
              {r.customs_updates && (
                <p className="text-sm mb-1">
                  <span className="font-medium">Customs:</span> {r.customs_updates}
                </p>
              )}
              {r.important_followups && (
                <p className="text-sm mb-1 bg-yellow-50 p-2 rounded">
                  <span className="font-medium">Follow-ups:</span> {r.important_followups}
                </p>
              )}
              {r.problems_issues && (
                <p className="text-sm mb-1 text-red-600">
                  <span className="font-medium">Issues:</span> {r.problems_issues}
                </p>
              )}
              {r.other_notes && (
                <p className="text-sm text-gray-600">
                  <span className="font-medium">Notes:</span> {r.other_notes}
                </p>
              )}
            </div>
          ))}
